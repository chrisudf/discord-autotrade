...
- **`fill_checker.confirm_buy_fill` 有一条完全静默的分支。** filled 分支只在
  `dealt != limit_price` 时回填并打日志;相等就直接 `return`——不回填、不打印、
  不告警。当晚 MU 那单全日志零 `[fill]` 行,而同晚 ASTS/SPCX 都在 15s 内出了
  `FILL_ADJUST`。"什么都没有"与"压根没跑"在日志上无法区分。
  修法两步:(a) filled 分支无论如何留一行;(b) **filled 之后再用
  `_get_long_qty(option_code)` 复核一次真实持仓**——`order_status` 说成交不等于
  真有仓,当晚 broker 的 `position_list_query` 从头到尾都是 0 长仓。对不上就
  告警并**不入账**。这是唯一能在 3 分钟内发现幻影仓的手段。
...
- **限价与市价的偏离没有任何闸门。** 当晚用 $2.54 去买一张市价 $14.80 的合约
  (信号说 weekly,parser 按规则解析成本周五 8/14,而喊单员指的是当天到期的
  8/12;他 8 分钟后自己发的更正 `$MU $945 8/12 calls $2.00 avg` 又 Parse failed)。
  现有闸门只看**成本**(`max_price` / `MAX_PRICE_PER_CONTRACT` / 日限额),
  没有一层比对"限价 vs 该合约当前报价"——而 TP 在 5 秒后就取到了 $14.80,
  说明报价当时拿得到。
  修法:下单前若能取到报价,`limit` 与 `last` 偏离超阈值(如 ±40%)拒单 + TG,
  文案里把两个价都写出来。这层同时是**到期日语义歧义的兜底**:"weekly 到底
  指哪个周五"在 parser 里猜不可靠,而"这个合约的价格跟信号价对不上"是可测的。

两条都属钱路(影响是否下单),按契约铁律 2 需要单独拍板后再动。
...
- **`send_telegram` 裸调用绕开 `notify()` 包装,日志里查不到发没发。**
  ...
  修法:错误路径统一走 `notify()`;或给 `send_telegram` 自己加一行 INFO。
  顺带**待确认**:reconciler 首轮漂移 TG 到底发没发(验证法见 review_2026-08-14 §4.3)。
